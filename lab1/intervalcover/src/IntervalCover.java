...
public class IntervalCover {
    ...
    public static ArrayList<Integer> cover(Interval interval, ArrayList<Interval> intervals) {
        ...
        // sort intervals based on their right value
        ...
        double maxRight = interval.a; // the furthest we've reached to the right

        // repeat until we've found a cover (also check if left & right are equal)
        while ((maxRight < interval.b) || (interval.a == interval.b)) {
            int found = -1; // not found


            // find furthest reaching interval among remaining intervals
